...
import {  useFetchBlogsQuery, useFetchDeleteBlogMutation } from '../redux/features/blog/blogApi';
...
const Manage = () => {
  ...
  const { data: blogs = [], error, isLoading, refetch } = useFetchBlogsQuery(query);
   const [fetchDeleteBlog] = useFetchDeleteBlogMutation()

   const handleDelect =  async(id) => {
    try {
       const res =  await fetchDeleteBlog(id).unwrap()
        alert(res.message)
